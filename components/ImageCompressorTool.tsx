...
export default function ImageCompressorTool({
  ...
  const compressImage = async (
    ...
    let startQuality =
      sizeRatio < 0.1 ? 0.2 : sizeRatio < 0.5 ? 0.5 : 0.8;
    ...
    while (iteration < 6) {
      ...
      if (currentBlob.size < targetSizeBytes * 0.85) {
        ...
        if (
          ...
          options.maxWidthOrHeight = Math.floor(
            options.maxWidthOrHeight * 0.8
          );
        }
      }
      iteration++;
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white flex flex-col">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 sm:gap-8">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-x-1 gap-y-1.5 text-sm">
              {POPULAR_SIZES.map((size, index) => {
                const label = formatSizeLabel(size); // "50KB" / "5MB"
                const href = getSizeHref(size);      // "/compress-to-50kb"
                const isLast = index === POPULAR_SIZES.length - 1;
                return (
                  <span
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-x-1 gap-y-1.5 text-sm">
              {[
                ...
                {
                  label: "Visa Photo Compressor",
                  href: "/visa-photo-compressor",
                },
              ].map((item, index, arr) => (
                <span
                  key={item.label}
      ...
      <footer className="border-t border-slate-100 mt-auto">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 text-center text-xs text-slate-400">
          © 2025 ExactSize. All rights reserved.
        </div>
      </footer>
    </div>
  );
