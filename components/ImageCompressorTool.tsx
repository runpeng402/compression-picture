...
export default function ImageCompressorTool({ initialTargetSize = "", titleOverride }: ImageCompressorProps) {
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    if (droppedFile && droppedFile.type.startsWith("image/")) {
      console.log("File Dropped:", droppedFile.name); // 调试日志
      processFile(droppedFile)
    } else {
      console.log("Drop failed: No file or invalid type");
    }
  }, [])

  const processFile = useCallback((selectedFile: File) => {
    console.log("Processing file:", selectedFile.name, selectedFile.size); // 调试日志
    setStatus("uploading")
    setProgress(0)
    setError(null)
    ...
    reader.onload = (e) => {
      ...
      setCompressedBlob(null)
      setCompressedSize(null)
      console.log("File loaded successfully"); // 调试日志
    }
    reader.onerror = () => {
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFile) {
        console.log("File Selected via Click:", selectedFile.name); // 调试日志
  ...
  const compressImage = async (file: File, targetKB: number, onProgress: (p: number) => void) => {
    ...
    let minQ = 0; let maxQ = 1; let currentBlob = file; let iteration = 0;
  ...
  const handleCompress = useCallback(async () => {
    ...
    if (isNaN(target) || target <= 0) {
      setError("Please enter a valid target size"); setTimeout(() => setError(null), 5000); return
    }
    ...
    try {
      ...
      setStatus("error"); setTimeout(() => setError(null), 7000)
    }
  }, [file, targetSize])
  ...
  const quickSizes = [
    { value: 20, label: "20 KB" }, { value: 50, label: "50 KB" }, { value: 100, label: "100 KB" }, { value: 200, label: "200 KB" },
    { value: 300, label: "300 KB" }, { value: 500, label: "500 KB" }, { value: 1024, label: "1 MB" }, { value: 2048, label: "2 MB" },
  ]
  const features = [
    { icon: TargetIcon, title: "Precise Control", desc: "Exact KB targeting" },
    { icon: ZapIcon, title: "Lightning Fast", desc: "Seconds to compress" },
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white flex flex-col">
      <header className="border-b border-slate-200/60 bg-white/80 backdrop-blur-sm sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-slate-900">ExactSize</span>
          </div>
          <span className="text-xs text-slate-400 font-medium hidden sm:block">Free Online Tool</span>
        </div>
      </header>
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full px-4 sm:px-6 py-6 sm:py-12">
        <div className="text-center mb-8 sm:mb-12">
          ...
          <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-slate-900 mb-3 tracking-tight">
            {titleOverride || "Compress Image to Exact Size"}
          </h1>
          <p className="text-slate-500 text-sm sm:text-base max-w-md mx-auto">
            Reduce your image to a precise file size in KB
          </p>
        </div>
            ...
            <div className="flex flex-col lg:flex-row gap-6 lg:gap-8">
              ...
              <div className="flex-1 lg:flex-[1.2]">
                <div
                  ...
                  className={`relative rounded-xl border-2 border-dashed transition-all duration-150 cursor-pointer overflow-hidden min-h-[220px] sm:min-h-[280px] flex items-center justify-center ${isDragging ? "border-blue-500 bg-blue-50/80" : file ? "border-slate-200 bg-slate-50/50" : "border-slate-200 hover:border-blue-400 hover:bg-blue-50/30 group"}`}
                >
                  <input type="file" accept="image/jpeg,image/png,image/webp" onChange={handleFileSelect} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" disabled={status === "compressing" || status === "uploading"} />
                  {status === "uploading" && (
                    <div className="absolute inset-0 bg-white/90 flex flex-col items-center justify-center z-10">
                      <LoaderIcon className="w-8 h-8 text-blue-500 mb-3" />
                      <p className="text-sm font-medium text-slate-700 mb-2">Processing...</p>
                      <div className="w-48 h-1.5 bg-slate-200 rounded-full overflow-hidden"><div className="h-full bg-blue-500 transition-all duration-150 ease-out" style={{ width: `${progress}%` }} /></div>
                    </div>
                  )}
                  {preview ? (
              ...
              <div className="flex-1 flex flex-col gap-5">
                <div>
                  <label className="block text-sm font-semibold text-slate-800 mb-2">Target Size</label>
                  <div className="relative">
                    <input type="number" placeholder="Enter size" value={targetSize} onChange={(e) => setTargetSize(e.target.value)} className="w-full h-12 px-4 pr-14 rounded-xl border-2 border-slate-200 bg-white text-slate-800 placeholder:text-slate-400 font-medium focus:outline-none focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all duration-150" disabled={status === "compressing"} />
                    <span className="absolute right-4 top-1/2 -translate-y-1/2 text-sm text-slate-400 font-semibold">KB</span>
                  </div>
                </div>
                <div>
                ...
                <button onClick={status === "done" ? handleDownload : handleCompress} disabled={!canCompress && status !== "done"} className={`mt-auto h-14 rounded-xl font-semibold flex items-center justify-center gap-2.5 transition-all duration-150 relative overflow-hidden ${canCompress || status === "done" ? "bg-gradient-to-r from-blue-600 to-indigo-600 text-white shadow-lg shadow-blue-500/25 hover:shadow-xl hover:shadow-blue-500/30 active:scale-[0.98]" : "bg-slate-100 text-slate-400 cursor-not-allowed"}`}>
                  {status === "compressing" && (<div className="absolute inset-0 bg-blue-700/30 transition-all duration-100 ease-out" style={{ width: `${progress}%` }} />)}
                  <span className="relative flex items-center gap-2.5">
                    {status === "compressing" ? (<><LoaderIcon className="w-5 h-5" /><span>Compressing... {progress}%</span></>) : status === "done" ? (<><DownloadIcon className="w-5 h-5" /><span>Download Result</span></>) : (<><ZapIcon className="w-5 h-5" /><span>Compress Now</span></>)}
                  </span>
                </button>
              </div>
            </div>
          ...
          <div className="bg-slate-50/80 border-t border-slate-100 px-4 sm:px-6 lg:px-8 py-5">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
              {features.map((feature, index) => (
                <div key={index} className="flex items-center gap-2.5 p-2.5 rounded-lg bg-white/60"><div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center shrink-0"><feature.icon className="w-4 h-4 text-slate-500" /></div><div className="min-w-0"><p className="text-xs font-medium text-slate-700 truncate">{feature.title}</p><p className="text-[10px] text-slate-400 truncate hidden sm:block">{feature.desc}</p></div></div>
              ))}
            </div>
          </div>
        </div>
      ...
      <section className="max-w-5xl mx-auto w-full px-4 sm:px-6 py-8 sm:py-12">
        <h2 className="text-lg font-semibold text-slate-800 mb-6">Popular Compressions</h2>
      ...
      <footer className="border-t border-slate-100 mt-auto">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-4 text-center text-xs text-slate-400">© 2025 ExactSize. All rights reserved.</div>
      </footer>
    </div>
  )
}
